...
- Duffel flight search is wired through `/api/flights` when `DUFFEL_ACCESS_TOKEN` is set. Creating flight offer requests requires a read/write test token; read-only tokens cannot search flights.
